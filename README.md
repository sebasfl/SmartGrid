...
- Supports **6-hour resampled data** for memory-efficient training on GPUs with <12GB VRAM
- Processes **27.7M records** from **1,578 buildings** (or ~4.6M with 6h resampling)
...
- **Sequences**: Lookback=240 intervals (60 days), Horizon=120 intervals (30 days) with 6h data
...
- GPU with 4-6GB+ VRAM (GTX 1650 Ti or better with 6h resampled data)

### Run Everything (Single Command)
```bash
docker compose build trainer-gpu && docker compose run --rm trainer-gpu bash -c "python -m src.ingest.fetch_bdg2 && python -m src.analysis.clean_building_data_parallel --parquet data/processed/bdg2_electricity_long.parquet --output data/processed/bdg2_cleaned_6h.parquet --resample-6h && python -m src.analysis.data_quality --parquet data/processed/bdg2_cleaned_6h.parquet --output_quality data/quality_report_6h.csv --output_split data/building_split_top_n.json --min_quality 0.99 --min_records 1460 --granularity 6 --top_n 50 && python -m src.main --parquet data/processed/bdg2_cleaned_6h.parquet --building_split data/building_split_top_n.json --model_dir models/cnn_lstm_6h_quality --epochs 50 --use_full_dataset"
```
...
```bash
docker compose build trainer-gpu
```
...
#### 3. Preprocessing (Time Features + Deduplication + Optional 6h Resampling)
```bash
# WITH 6-hour resampling (RECOMMENDED - reduces memory by 83%)
docker compose run --rm trainer-gpu python -m src.analysis.clean_building_data_parallel --parquet data/processed/bdg2_electricity_long.parquet --output data/processed/bdg2_cleaned_6h.parquet --resample-6h
...
```bash
docker compose run --rm trainer-gpu python -m src.analysis.data_quality --parquet data/processed/bdg2_cleaned_6h.parquet --output_quality data/quality_report_6h.csv --output_split data/building_split_top_n.json --min_quality 0.99 --min_records 1460 --granularity 6 --top_n 50
```

#### 5. Train CNN-LSTM Model
```bash
# With 6h data + high-quality buildings (RECOMMENDED)
docker compose run --rm trainer-gpu python -m src.main --parquet data/processed/bdg2_cleaned_6h.parquet --building_split data/building_split_top_n.json --model_dir models/cnn_lstm_6h_quality --epochs 50 --use_full_dataset

# Quick test (2 epochs)
docker compose run --rm trainer-gpu python -m src.main --parquet data/processed/bdg2_cleaned_6h.parquet --building_split data/building_split_top_n.json --model_dir models/cnn_lstm_6h_quality --epochs 2 --use_full_dataset
```

#### 6. Batch Inference (Prediction)
```bash
docker compose run --rm trainer-gpu python -m src.predict --model_dir models/cnn_lstm_6h_quality --parquet data/processed/bdg2_cleaned_6h.parquet --building_split data/building_split_top_n.json --output predictions.csv
```
...
- **6-hour resampling** for memory-efficient training on limited VRAM
...
### GPU Memory Issues
```bash
# Use 6h resampled data (recommended)
# Reduce batch size
...
**Typical training time (full dataset, RTX 3060, 6h resampled data):**
...
- **Size**: 27.7M hourly records (~4.6M with 6h resampling)
