...
def evaluate_all_buildings_gpu(gdf: cudf.DataFrame, granularity_hours: int = 1) -> pd.DataFrame:
    ...
    time_diff_seconds = (building_stats["timestamp_local_max"] - building_stats["timestamp_local_min"]).astype(
        "int64"
    ) / 1_000_000_000
    ...
    missing_counts = (
        gdf["value"].isna().groupby(gdf["building_id"]).sum().reset_index().rename(columns={"value": "missing_count"})
    )
    ...
    building_stats["expected_records"] = (building_stats["time_span_hours"].astype("int64") / granularity_hours).astype(
        "int64"
    ) + 1
    ...
    zero_counts = (
        (gdf["value"] == 0).groupby(gdf["building_id"]).sum().reset_index().rename(columns={"value": "zero_count"})
    )
    zero_counts.columns = ["building_id", "zero_count"]
    building_stats = building_stats.merge(zero_counts, on="building_id")
    building_stats["zero_ratio"] = building_stats["zero_count"].astype("float64") / building_stats[
        "value_count"
    ].astype("float64")
    ...
    building_stats["continuity_ratio"] = (
        1.0 - building_stats["gaps_2h"].astype("float64") / (building_stats["value_count"].astype("float64") - 1.0)
    ).fillna(0.0)

    gdf["is_dup"] = gdf.sort_values(["building_id", "timestamp_local"]).duplicated(
        subset=["building_id", "timestamp_local"]
    )
    dup_counts = (
        gdf["is_dup"].groupby(gdf["building_id"]).sum().reset_index().rename(columns={"is_dup": "duplicate_timestamps"})
    )
    dup_counts.columns = ["building_id", "duplicate_timestamps"]
    building_stats = building_stats.merge(dup_counts, on="building_id")
    building_stats["duplicate_ratio"] = building_stats["duplicate_timestamps"].astype("float64") / building_stats[
        "value_count"
    ].astype("float64")
    ...
    building_stats["constant_ratio"] = building_stats["constant_sequences"].astype("float64") / building_stats[
        "value_count"
    ].astype("float64")
...
def _log_quality_summary(quality_df: pd.DataFrame) -> None:
    ...
    logger.info(
        "Quality distribution: High(>=0.8): %d (%.0f%%) | Medium: %d (%.0f%%) | Low: %d (%.0f%%)",
        high,
        100 * high / total,
        medium,
        100 * medium / total,
...
def select_high_quality_buildings(
    ...
    logger.info(
        "Selected %d/%d buildings (score>=%.2f, records>=%d)",
        len(selected),
        len(quality_df),
        min_quality_score,
        min_records,
...
def split_buildings_for_training(
    ...
    logger.info("Split: %d train / %d val / %d test", len(split["train"]), len(split["validation"]), len(split["test"]))
    return split
...
def load_building_split(input_path: str) -> dict[str, list[str]]:
    with open(input_path) as f:
        split = json.load(f)
    logger.info(
        "Loaded split: %d train / %d val / %d test",
        len(split.get("train", [])),
